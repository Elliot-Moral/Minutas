...
$(function(){
	$('#DivPuesto').css({position : 'absolute'});
});
function FiltrarPuesto(){
	mFiltro = document.getElementById('FiltroPuesto').value.toUpperCase();
	if(mFiltro){
		$("#TBodyPuesto").find("tr").each(function(){
		...
		$("#TBodyPuesto").find("tr").show();
	}
}
function MostrarDivPuesto(mIDPuesto){
	$("#FrmPuesto").find("input:text,select,textarea").each(function(){
		this.classList.remove(...estilos.requeired);
	});//Ojo, solo se deben incluir en input los text, porque se pierden los efectos de los botones
	document.getElementById('IDPuesto').value = mIDPuesto;
	if(mIDPuesto=='Nuevo'){
		...
		$.ajax({
			type: "GET",
			url: 'index.php',
			data: 'TipoModificar=<?php echo md5('Ajax1JorA2Puesto'.date('d'));?>&IDPuesto='+mIDPuesto,
			cache: false,
			dataType: 'json',
			success: function(data){ //Si se ejecuta correctamente
              ...
              position: "top-end",
              icon: "error",
              title: 'No se pudieron cargar los datos del puesto, por favor intente nuevamente.',
			...
			error: function(data){
              ...
              position: "top-end",
              icon: "error",
              title: 'Se presentó un error, por favor intente nuevamente.',
              showConfirmButton: false,
              timer: 3000
					});
				return false;
	...
	MostrarDivModales('Puestos');
}
function EnviarPuesto(){//Enviar los datos del pago en un ajax
	$("#BotsPuesto").hide();
	mRetorno=true;
	ele=document.getElementById('Puesto');if(ele.value){ele.classList.remove(...estilos.requeired);}else{ele.classList.add(...estilos.requeired);mRetorno=false;}
	if(mRetorno){
		document.getElementById('FrmPuesto').TipoGrabar.value='A2';
		document.getElementById('FrmPuesto').TipoModificar.value='<?php echo md5('JorA2'.date('d'));?>';
		var myData = $("#FrmPuesto").serialize();
		$.ajax({
			url:'index.php',
			type:'post',
			cache: false,
...
<section class="h-screen col-span-9 md:col-span-8 bg-gray-50 p-4 lg:p-8 S max-h-screen"><?php
		...
		<h1 class="text-2xl font-bold mb-0 text-gray-600">Gestión De Puestos</h1>
  	<hr>
		...
		<div class="flex items-center gap-x-2 mt-8">
			...
			<div>
				<input name="FiltroPuesto" type="text" class="block w-full max-w-32 ps-3 pe-3 py-2 text-gray-500 rounded-lg border border-default-medium text-heading text-sm shadow-xs placeholder:text-body outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-all" 
				id="FiltroPuesto" placeholder="Filtrar Puesto" onChange="FiltrarPuesto();">
			</div>
			<div>
				<button onClick="FiltrarPuesto();"
			...
			<div>
				<div onClick="MostrarDivPuesto('Nuevo');" title="Crear registro"
					class="cursor-pointer bg-gradient-to-br from-blue-700 to-blue-400 px-4 py-2 rounded-lg text-white hover:shadow-lg active:scale-95 transition-all font-semibold flex items-center gap-x-2">
					<svg  xmlns="http://www.w3.org/2000/svg" width="19" height="19" fill="#ffffff" viewBox="0 0 24 24" >
	...
	<div class="mt-4 rounded-t-xl border border-gray-200 text-sm text-gray-600 overflow-x-auto h-[75vh] md:h-[72vh]">
		...
		<table class="table table-striped table-bordered">
			<thead class="bg-gray-100  rounded-t-lg shadow-xs sticky top-0 z-10">
				<tr>
						...
						<th width="70%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Puesto</th>
						<th width="15%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Estado</th>
						<th width="15%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Acciones</th>
				</tr>
			</thead>
			<tbody id="TBodyPuesto">
				<?php
				while($Row = $Result->fetch_assoc()){?>
					<tr class="text-center bg-white border-b border-gray-200">
						...
						<td class="p-3"><?php echo $Row['Puesto'];?></td>
						<td class="p-3">
						...
						<td class="p-3 flex justify-center items-center gap-x-2">
							<?php
							if($PuedeAdministrar){?>
									<span onClick="MostrarDivPuesto(<?php echo $Row['IDPuesto'];?>);" class="cursor-pointer group" title="Editar registro">
...
<!-- MODAL CREAR Y EDITAR PUESTO -->
 <div id="modal-Puestos" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 font-normal">
    <div id="modal-Puestos-backdrop" class="modal-backdrop absolute inset-0 bg-black/30 " onclick="CerrarDivModales('Puestos')"></div>
    <div id="modal-Puestos-container" class="relative bg-white rounded-2xl shadow-2xl w-full md:max-w-[40%] overflow-hidden">
        <div class="bg-gradient-to-br from-blue-600 to-blue-400 p-6 text-white">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-x-3">
                    ...
                    <div>
                        <h3 id="modal-Puestos-title" class="text-xl">Editar | Crear Puesto</h3>
                        <p class="text-blue-100 text-sm"></p>
                    </div>
                </div>

                <button onclick="CerrarDivModales('Puestos')" class="hover:bg-white/20 p-2 rounded-lg transition-colors cursor-pointer">
            ...
            <div class="grid grid-cols-1 gap-4">
							...
							<div class="">
									<form method="post" enctype="multipart/form-data" name="FrmPuesto" id="FrmPuesto" >
										<!-- Inputs Ocultos -->
									...
									<input name="IDPuesto" type="hidden" id="IDPuesto">
										...
										<div class="flex flex-col gap-4">
											...
											<div class="md:col-span-4 flex flex-col gap-1.5 text-left">
												<label for="Puesto" class="">Descripción del puesto</label>
												<input name="Puesto" type="text" id="Puesto"
						...
						<div class="flex gap-3 pt-4" id="BotsPuesto">
								<button type="button" onclick="CerrarDivModales('Puestos')"
								...
								<button  type="button" id="BotAceptarCerrarMinuta" onclick="EnviarPuesto();"
