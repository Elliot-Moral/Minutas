...
$(function(){
	$('#DivElemento').css({position : 'absolute'});
});
function FiltrarElemento(){
	mFiltro = document.getElementById('FiltroElemento').value.toUpperCase();
	if(mFiltro){
		$("#TBodyElemento").find("tr").each(function(){
		...
		$("#TBodyElemento").find("tr").show();
	}
}
function MostrarDivElemento(mIDElemento){
	$("#FrmElemento").find("input:text,select,textarea").each(function(){
		this.classList.remove(...estilos.requeired);
	});
	document.getElementById('IDElemento').value = mIDElemento;
	if(mIDElemento=='Nuevo'){
		...
		$.ajax({
			type: "GET",
			url: 'index.php',
			data: 'TipoModificar=<?php echo md5('Ajax1JorA4Elemento'.date('d'));?>&IDElemento='+mIDElemento,
			cache: false,
			dataType: 'json',
			success: function(data){ //Si se ejecuta correctamente
              ...
              timer: 3000
					...
					document.getElementById('Elemento').value=data.Elemento;
	...
	MostrarDivModales('Elemento');
}
function EnviarElemento(){//Enviar los datos del pago en un ajax
	$("#BotsElemento").hide();
	mRetorno=true;
	ele=document.getElementById('Elemento');if(ele.value){ele.classList.remove(...estilos.requeired);}else{ele.classList.add(...estilos.requeired);mRetorno=false;}
	ele=document.getElementById('Grupo');if(ele.value){ele.classList.remove(...estilos.requeired);}else{ele.classList.add(...estilos.requeired);mRetorno=false;}
	if(mRetorno){
		document.getElementById('FrmElemento').TipoGrabar.value='A4';
		document.getElementById('FrmElemento').TipoModificar.value='<?php echo md5('JorA4'.date('d'));?>';
		var myData = $("#FrmElemento").serialize();
		$.ajax({
			url:'index.php',
			type:'post',
			cache: false,
...
<section class="h-screen col-span-9 md:col-span-8 bg-gray-50 p-4 lg:p-8 S max-h-screen"><?php
	...
	<h1 class="text-2xl font-bold mb-0 text-gray-600">Administrar Elementos</h1>
  <hr>
	...
	<div class="flex  gap-2 mt-8">
		...
		<div>
			<input name="FiltroElemento" type="text" class="block w-full max-w-64 ps-3 pe-3 py-2 text-gray-500 rounded-lg border border-default-medium text-heading text-sm shadow-xs placeholder:text-body outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-all"  
	...
	<div class="mt-4 rounded-t-xl border border-gray-200 text-sm text-gray-600 overflow-x-auto h-[77vh] md:h-[72vh]">
		<table class="table table-striped table-bordered">
			<thead class="bg-gray-100  rounded-t-lg shadow-xs sticky top-0 z-10">
					<tr>
					<th width="8%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">ID</th>
					<th width="50%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Elemento</th>
					<th width="34%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Grupo</th>
					<th width="8%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Estado</th>
					<th width="8%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Acciones</th>
				</tr>
				</thead>
				<tbody id="TBodyElemento">
			<?php
			while($Row = $Result->fetch_assoc()){?>
				<tr  class="bg-white border-t border-gray-200" align=center>
						...
						<div class="inline-flex items-center gap-3 px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg group hover:bg-white transition-colors">
							<div class="flex flex-col">
								<span class="text-[12px] font-mono font-bold text-slate-700 leading-none tracking-tight">
										<?php echo $Row['IDElemento'];?>
								</span>
							</div>
						</div>
					</td>
					<td class="p-2" align="left"><?php echo $Row['Elemento'];?></td>
					...
					<td class="p-2">
						<?php
							if($PuedeAdministrar){?>
							<span onClick="MostrarDivElemento(<?php echo $Row['IDElemento'];?>);" class="cursor-pointer group" title="Editar registro">
...
<!-- MODAL ELEMENTOS -->
 <div id="modal-Elemento" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 font-normal">
    <div id="modal-Elemento-backdrop" class="modal-backdrop absolute inset-0 bg-black/30 " onclick="CerrarDivModales('Elemento')"></div>
    <div id="modal-Elemento-container" class="relative bg-white rounded-2xl shadow-2xl w-full md:max-w-[40%] overflow-hidden">
        <div class="bg-gradient-to-br from-blue-600 to-blue-400 p-6 text-white">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-x-3">
                    ...
                    <div>
                        <h3 id="modal-Elemento-title" class="text-xl">Editar | Crear Elemento</h3>
                        <p class="text-blue-100 text-sm"></p>
                    </div>
                </div>

                <button onclick="CerrarDivModales('Elemento')" class="hover:bg-white/20 p-2 rounded-lg transition-colors cursor-pointer">
            ...
            <div class="grid grid-cols-1 gap-4">
							...
							<div class="">
									<form method="post" enctype="multipart/form-data" name="FrmElemento" id="FrmElemento" >
										<!-- Inputs Ocultos -->
									...
									<input name="IDElemento" type="hidden" id="IDElemento">
										...
										<div class="flex flex-col gap-4">
											...
											<div class="md:col-span-4 flex flex-col gap-1.5 text-left">
												<label for="Elemento" class="">Elemento</label>
						...
						<div class="flex gap-3 pt-4" id="BotsElemento">
								<button type="button" onclick="CerrarDivModales('Elemento')"
								...
								<button  type="button" id="BotAceptarElemento" onClick="javascript:return EnviarElemento();"
