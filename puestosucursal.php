...
$(function(){
	$('#DivPuestoSucursal').css({position : 'absolute'});
});
function FiltrarPuestoSucursal(){
	mFiltroSucursal = document.getElementById('FiltroSucursal').value;
	mFiltroPuesto = document.getElementById('FiltroPuesto').value.toUpperCase();
	if(mFiltroSucursal || mFiltroPuesto){
		$("#TBodyPuestoSucursal").find("tr").each(function(){
			if((mFiltroSucursal ? ($(this).find("td:eq(1):contains('"+mFiltroSucursal+"')").length) : true)	&&
			   (mFiltroPuesto ? ($(this).find("td:eq(2):contains('"+mFiltroPuesto+"')").length) : true)){
				$(this).show();
			}else{
				$(this).hide();
			}
		...
		$("#TBodyPuestoSucursal").find("tr").show();
	}
}
function MostrarDivPuestoSucursal(mIDPuestoSucursal){
	$("#FrmPuestoSucursal").find("input:text,select,textarea").each(function(){
		this.classList.remove(...estilos.requeired);
	});
	document.getElementById('IDPuestoSucursal').value = mIDPuestoSucursal;
	if(mIDPuestoSucursal=='Nuevo'){
		document.getElementById('Sucursal').value='';
		document.getElementById('IDPuesto').value='';
		document.getElementById('ObsPuesto').value='';
		document.getElementById('Borrada').value=0;
	}else{
		$.ajax({
			type: "GET",
			url: 'index.php',
			data: 'TipoModificar=<?php echo md5('Ajax1JorA3PuestoSucursal'.date('d'));?>&IDPuestoSucursal='+mIDPuestoSucursal,
			cache: false,
			dataType: 'json',
			success: function(data){ //Si se ejecuta correctamente
				if(data.Mensaje=="Error"){
					...
					document.getElementById('ObsPuesto').value=data.ObsPuesto;
	...
	MostrarDivModales('PuestoSucursal');
}
function EnviarPuestoSucursal(){//Enviar los datos del pago en un ajax
	mRetorno=true;
	...
	ele=document.getElementById('IDPuesto');if(ele.value){ele.classList.remove(...estilos.requeired);}else{ele.classList.add(...estilos.requeired);mRetorno=false;}
	ele=document.getElementById('ObsPuesto');if(ele.value){ele.classList.remove(...estilos.requeired);}else{ele.classList.add(...estilos.requeired);mRetorno=false;}
	if(mRetorno){
		$("#BotsPuestoSucursal").hide();
		document.getElementById('FrmPuestoSucursal').TipoGrabar.value='A3';
		document.getElementById('FrmPuestoSucursal').TipoModificar.value='<?php echo md5('JorA3'.date('d'));?>';
		var myData = $("#FrmPuestoSucursal").serialize();
		$.ajax({
			url:'index.php',
			type:'post',
			cache: false,
...
<section class="h-screen col-span-9 md:col-span-8 bg-gray-50 p-4 lg:p-8 S max-h-screen">
	...
	<h1 class="text-2xl font-bold mb-0 text-gray-600">Administrar puestos por sucursal</h1>
	<hr>

	<div class="grid grid-cols-3 md:flex gap-2 mt-8">
		<div class="relative">
		...
		<div>
				<select name="FiltroSucursal" id="FiltroSucursal" class="block w-full md:max-w-64 ps-3 pe-3 py-2 text-gray-500 rounded-lg border border-default-medium text-heading text-sm shadow-xs placeholder:text-body outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-all"  
				onChange="FiltrarPuestoSucursal();">
		...
		<div>
			<input name="FiltroPuesto" type="text" class="block w-full md:max-w-64 ps-3 pe-3 py-2 text-gray-500 rounded-lg border border-default-medium text-heading text-sm shadow-xs placeholder:text-body outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-all"  
			id="FiltroPuesto" placeholder="Filtrar Puesto" onChange="FiltrarPuestoSucursal();">
		</div>
		<div>
				<button onClick="FiltrarPuestoSucursal();"
		...
		<div>
				<div onClick="MostrarDivPuestoSucursal('Nuevo');"
					class="w-full md:w-fit cursor-pointer bg-gradient-to-br from-blue-700 to-blue-400 px-4 py-2 rounded-lg text-white hover:shadow-lg active:scale-95 transition-all font-semibold flex items-center gap-x-2">
					<svg  xmlns="http://www.w3.org/2000/svg" width="19" height="19" fill="#ffffff" viewBox="0 0 24 24" >
	...
	<div class="mt-4 rounded-t-xl border border-gray-200 text-sm text-gray-600 overflow-x-auto h-[73vh] md:h-[72vh]">
		<table class="table table-striped table-bordered">
			<thead class="bg-gray-100  rounded-t-lg shadow-xs sticky top-0 z-10">
					<tr>
						<th width="6%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">ID</th>
						<th width="28%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Sucursal</th>
						<th width="28%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Puesto</th>
						<th width="28%" class="p-3 text-left text-sm font-bold text-gray-700 tracking-wider text-center">Observación Puesto</th>
			...
			<tbody id="TBodyPuestoSucursal">
					<?php
						$Queri = "SELECT PuestoSucursal.*, Sucursal.NomSucursal, Puesto.Puesto
							...
							ORDER BY Sucursal.Sucursal, Puesto.Puesto";
						$Result = $mysqli->query($Queri) or die(mysqli_error($mysqli));
							while($Row = $Result->fetch_assoc()){?>
								<tr class="bg-white border-t border-gray-200">
											...
											<div class="inline-flex items-center gap-3 px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg group hover:bg-white transition-colors">
													<div class="flex flex-col">
															<span class="text-[12px] font-mono font-bold text-slate-700 leading-none tracking-tight">
																	<?php echo $Row['IDPuestoSucursal'];?>
															</span>
													</div>
											</div>
									</td> 
									<td class="p-2"><?php echo $Row['Sucursal']." ".$Row['NomSucursal'];?></td>
									<td class="p-2"><?php echo $Row['Puesto'];?></td>
									<td class="p-2"><?php echo $Row['ObsPuesto'];?></td>
									<td class="p-2" align="center">
									...
									<td align="center">
										<?php if($PuedeAdministrar){?>
											<span onClick="MostrarDivPuestoSucursal(<?php echo $Row['IDPuestoSucursal'];?>);" class="cursor-pointer group" title="Editar registro">
...
<!-- MODAL CREAR Y EDITAR PUESTO -->
 <div id="modal-PuestoSucursal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 font-normal">
    <div id="modal-PuestoSucursal-backdrop" class="modal-backdrop absolute inset-0 bg-black/30 " onclick="CerrarDivModales('PuestoSucursal')"></div>
    <div id="modal-PuestoSucursal-container" class="relative bg-white rounded-2xl shadow-2xl max-w-[95%] lg:max-w-[70%] w-full overflow-hidden">
        <div class="bg-gradient-to-br from-blue-600 to-blue-400 p-6 text-white">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-x-3">
                    ...
                    <div>
                        <h3 id="modal-PuestoSucursal-title" class="text-xl">Editar | Crear Puesto por Sucursal</h3>
                        <p class="text-blue-100 text-sm"></p>
                    </div>
                </div>

                <button onclick="CerrarDivModales('PuestoSucursal')" class="hover:bg-white/20 p-2 rounded-lg transition-colors cursor-pointer">
            ...
            <div class="grid grid-cols-1 gap-4">
							...
							<!-- Formulario de crear/editar Puestos -->
							<div class="">
									<form method="post" enctype="multipart/form-data" name="FrmPuestoSucursal" id="FrmPuestoSucursal" >
										<!-- Inputs Ocultos -->
									<input name="TipoGrabar" type="hidden" id="TipoGrabar">
									<input name="TipoModificar" type="hidden" id="TipoModificar">
									<input name="IDPuestoSucursal" type="hidden" id="IDPuestoSucursal">
										...
										<div class="flex flex-col gap-4">
											...
											<div class="flex flex-col md:flex-row gap-4">
												<div class="w-full">
													...
													<select name="Sucursal" type="text" id="Sucursal"
												...
												<div class="w-full">
													<label for="IDPuesto" class="">Puesto</label>
													<select name="IDPuesto" type="text" id="IDPuesto"
													class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none">
														<option value= '' selected>--Puesto--</option><?php
															$Queri = "SELECT Puesto.IDPuesto, Puesto.Puesto
																		FROM ".$PrefBD."solicitudes.vigilanciapuesto Puesto
																		ORDER BY Puesto.Puesto";
															$Result = $mysqli->query($Queri) or die(mysqli_error($mysqli));
															while($Row = $Result->fetch_assoc()){?>
																	<option value= '<?php echo $Row['IDPuesto'];?>'><?php echo $Row['Puesto'];?></option><?php
															}?>
													</select>
												</div>
											</div>
											...
											<div class="flex flex-col md:flex-row gap-4">
												<div class="w-full">
														<label for="ObsPuesto">Descripción detallada del puesto</label>
														<input name="ObsPuesto" id="ObsPuesto" maxlength="250" title="Descripción detallada del puesto"
														class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" >
												</div>
						...
						<div class="flex gap-3 pt-4" id="BotsPuestoSucursal">
								<button type="button" onclick="CerrarDivModales('PuestoSucursal')"
								...
								<button  type="button" id="BotAceptarPuestoSucursal" onClick="javascript:return EnviarPuestoSucursal();"
